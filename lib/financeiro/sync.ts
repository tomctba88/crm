...
export async function syncContasReceber(supabase: SupabaseClient, token: string) {
  ...
  const records = itens
    ...
    .map(item => ({
      ...
      data_recebimento: parseDateBR(str(item.data_ocorrencia ?? item.data_vencimento ?? item.data_emissao)),
...
export async function syncContasPagar(supabase: SupabaseClient, token: string) {
  ...
  const records = itens
    ...
    .map(item => ({
      ...
      data_vencimento: parseDateBR(str(item.data_vencimento)),
      data_pagamento: parseDateBR(str(item.data_ocorrencia ?? item.data_vencimento ?? item.data_emissao)),
