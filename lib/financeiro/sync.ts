...
export async function syncContasReceber(supabase: SupabaseClient, token: string) {
  ...
  const records = itens
    ...
    .map(item => {
      ...
      // Use data_vencimento as the payment date reference.
      // This correctly distributes received accounts by their due month
      // instead of all accumulating on the sync date.
      const dataRef = parseDateBR(str(item.data_vencimento ?? item.data_emissao))
      return {
        tiny_id: str(item.id),
        ...
        data_recebimento: novoStatus === 'recebido' ? dataRef : parseDateBR(str(item.data_vencimento)),
        status: novoStatus,
  ...
  const records = itens
    ...
    .map(item => {
      ...
      return {
        ...
        data_vencimento: parseDateBR(str(item.data_vencimento)),
        data_pagamento: novoStatus === 'pago' ? dataRef : parseDateBR(str(item.data_vencimento)),
        status: novoStatus,
        categoria: str(item.categoria),
        conta_bancaria: str(item.conta_bancaria),
        observacoes: str(item.historico ?? item.observacoes),
...
// Derives cash flow from local synced tables — avoids depending on a Tiny
// endpoint that may not exist. Uses data_vencimento as the date reference.
...
// Stub kept for the /sincronizar/caixa route — may be replaced with real
// caixa.pesquisa logic once the Tiny endpoint is confirmed to exist.
export async function syncCaixa(
  supabase: SupabaseClient,
  _token: string,
  _dataInicialISO: string,
  _dataFinalISO: string,
) {
  return syncFluxoCaixa(supabase)
}
