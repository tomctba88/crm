...
export default function ContasReceberManager() {
  ...
  const carregar = useCallback(async () => {
    ...
    setLoading(false)
  }, [filtroStatus, filtroCliente, filtroInicio, filtroFim, filtroCategoria, ordenarPor, asc, pagina])
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
        {[
          { label: 'Total em Aberto', valor: totalAberto, cor: 'text-[#1b4fd6]' },
      ...
      <div className="flex flex-wrap gap-3 rounded-2xl bg-white p-4 shadow-sm border border-slate-200">
        ...
        <input value={filtroCategoria} onChange={e => { setFiltroCategoria(e.target.value); setPagina(0) }}
          ...
          className="flex-1 min-w-36 rounded-xl border border-slate-200 bg-[#eef3fb] px-3 py-2 text-sm outline-none focus:border-[#1b4fd6]" />
        <button onClick={() => { setFiltroStatus('todos'); setFiltroCliente(''); setFiltroInicio(''); setFiltroFim(''); setFiltroCategoria(''); setPagina(0) }}
