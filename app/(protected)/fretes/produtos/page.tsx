...
export default function ProdutosPage() {
  ...
  async function buscarProdutos() {
    setLoading(true)
    const { data } = await supabase.from('frete_produtos').select('id,nome,largura,comprimento,altura,peso').order('id')
    setLoading(false)
    setProdutos(data || [])
  }
  ...
  async function salvarOuAtualizar() {
    ...
    const payload = { nome: nome.trim(), largura: Number(largura.replace(',', '.')), comprimento: Number(comprimento.replace(',', '.')), altura: Number(altura.replace(',', '.')), peso: peso ? Number(peso.replace(',', '.')) : null }
    if (editandoId) {
  ...
  function editar(p: Produto) {
    setNome(p.nome); setLargura(p.largura != null ? String(p.largura) : ''); setComprimento(p.comprimento != null ? String(p.comprimento) : '')
    setAltura(p.altura != null ? String(p.altura) : ''); setPeso(p.peso != null ? String(p.peso) : ''); setEditandoId(p.id)
  }

  function limpar() { setNome(''); setLargura(''); setComprimento(''); setAltura(''); setPeso(''); setEditandoId(null) }
      ...
      <div className={card}>
        ...
        <div className="overflow-x-auto">
          <table style={{ borderCollapse: 'collapse', width: '100%', minWidth: '700px' }}>
            <thead>
              <tr>{['ID','Nome','Largura (mm)','Comprimento (mm)','Altura (mm)','Peso (kg)','Cubagem Unit. (m³)','Ações'].map((h) => <th key={h} style={th}>{h}</th>)}</tr>
            </thead>
            <tbody>
              ...
              {!loading && produtosFiltrados.length === 0 && <tr><td style={td} colSpan={8} className="text-center text-slate-400">Nenhum produto encontrado.</td></tr>}
            </tbody>
          </table>
        </div>
      </div>
